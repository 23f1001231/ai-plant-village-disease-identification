...
async def voice_speech_to_text(
    ...
    if stt_service is not None:
        transcription = stt_service.transcribe_audio(audio_bytes, language, filename=file.filename)
...
def voice_text_to_speech(
    ...
    if tts_service is not None:
        return StreamingResponse(tts_service.stream_speech(text, language), media_type="audio/mp3")
